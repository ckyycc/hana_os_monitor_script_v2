...
class TestConfigMonitor(TestCase):
    def setUp(self):
        ...
        self.config_msg = {self.cpu_threshold: 1, self.mem_threshold: 2, self.disk_threshold: 3,
                           self.email_sender: "test@t.com", Mc.DB_CONFIGURATION_SERVER: self.config_server_list}

    def test_initialize(self):
        config_monitor, mock_dao, mock_producer = self.__get_mock_objects()
        mock_dao.return_value.get_configuration.return_value = self.config_list
        mock_dao.return_value.get_server_full_names.return_value = self.server_list
        config_monitor._ConfigMonitor__monitoring_configurations(mock_dao.return_value)

        mock_dao.return_value.get_configuration.assert_called_once()
        mock_dao.return_value.get_server_full_names.assert_called_once()
        mock_producer.return_value.send.assert_called_once_with(Mc.TOPIC_CONFIGURATION, self.config_msg)
    ...
    def test_config_change(self):
        ...
        mock_dao.return_value.get_configuration.return_value = config_list
        server_list = self.server_list.copy()
        server_list[0][2] = "/data"  # change mount point
        mock_dao.return_value.get_server_full_names.return_value = server_list
        ...
        mock_producer.return_value.send.assert_called_with(Mc.TOPIC_CONFIGURATION, config_msg)
    ...
    def __get_mock_objects(self, mock_producer, mock_dao):
        ...
        return config_monitor, mock_dao, mock_producer
